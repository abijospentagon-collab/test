...
"2. CREATE_TASK:\n" .
"   - If no specific task or assignee is provided, default to {\"type\": \"CREATE_TASK\", \"title\": \"General housekeeping\", \"assignedTo\": \"ST203\", \"category\": \"Cleaning\", \"priority\": \"Medium\"}\n" .
...
if ($groqSuccess && $parsedResponse) {
    ...
    // 2. CREATE TASK
    elseif (preg_match('/create\s+(a\s+)?task|assign\s+(a\s+)?task|new\s+task|add\s+(a\s+|new\s+)?task|set\s+(a\s+)?task/i', $rawText)) {
        $assigneeId = 'ST203'; // Default: Savitri Devi
        $assigneeName = 'Savitri Devi';
        $assigneeTamil = 'சாவித்திரி தேவி';

        // Detect staff member from spoken name
        if (preg_match('/ramesh|kitchen|maintenance/i', $rawText)) {
            $assigneeId = 'ST205';
            $assigneeName = 'Ramesh Kumar';
            $assigneeTamil = 'ரமேஷ் குமார்';
        } elseif (preg_match('/priya|lkg teacher/i', $rawText)) {
            $assigneeId = 'ST201';
            $assigneeName = 'Priya Sen';
            $assigneeTamil = 'பிரியா சென்';
        } elseif (preg_match('/anita|ukg teacher/i', $rawText)) {
            $assigneeId = 'ST204';
            $assigneeName = 'Anita Kapoor';
            $assigneeTamil = 'அனிதா கபூர்';
        } elseif (preg_match('/admin|rohan/i', $rawText)) {
            $assigneeId = 'ST202';
            $assigneeName = 'Rohan Das';
            $assigneeTamil = 'ரோஹன் தாஸ்';
        }

        // Parse task title from command
        $taskTitle = '';
        // Try extracting title after keywords like "task for Ramesh [title]" or "task to [title]"
        if (preg_match('/task\s+(?:for|to|titled?|called|named)\s+(?:\w+\s+)?(.+)/i', $transcript, $titleMatch)) {
            $candidate = trim(preg_replace('/\b(savitri|ramesh|priya|anita|rohan|kumar|devi|sen|kapoor|das)\b/i', '', $titleMatch[1]));
            $candidate = trim($candidate);
            if (strlen($candidate) > 3) $taskTitle = ucfirst($candidate);
        }
        // Fallback: extract text after staff name
        if (!$taskTitle) {
            $afterName = preg_replace('/^.*?(ramesh|savitri|priya|anita|rohan)\s*/i', '', $transcript);
            if (strlen(trim($afterName)) > 3 && strtolower(trim($afterName)) !== strtolower($transcript)) {
                $taskTitle = ucfirst(trim($afterName));
            }
        }
        // Final fallback title
        if (!$taskTitle || strlen($taskTitle) < 3) {
        ...
        $newTask = [
          "id"          => "T" . rand(100, 999),
          "title"       => $taskTitle,
          "description" => "Voice-assigned task: " . $taskTitle,
